...
| 模型     | 数据集   | 预测视野 | MAE (归一化) | RMSE (归一化) | MAE (mph) | RMSE (mph) |
|----------|----------|----------|--------------|---------------|-----------|------------|
| A3T-GCN  | METR-LA  | 12×5min  | 0.4462       | 0.7494        | 9.00      | 15.11      |

> 速度特征均值约 53.6 mph、标准差约 20.2 mph;真实速度尺度的指标由归一化误差
> 乘以标准差还原得到。使用全量数据并增大 epoch 可进一步降低误差。
